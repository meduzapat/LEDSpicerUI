...
void BoxButtonCollection::wipe() noexcept {
	// PROP_SYSTEM items outlive a project wipe; their child contents are
	// cleared by the existing dependency cascade when source collections wipe.
	BoxButtonVector survivors;
	for (auto item : items) {
		if (item->getData()->getProperties().isSet(PROP_SYSTEM))
			survivors.push_back(item);
		else
			delete item;
	}
	items.swap(survivors);
	refreshSensitiveWidgets();
	if (owner) owner->sync();
}
