...
protected:
	...
	GladeDialog(BaseObjectType* obj, const Glib::RefPtr<Gtk::Builder>&) : Gtk::Dialog(obj) {
		// Hide the CSD [x] close button uniformly. Every dialog provides an
		// explicit labelled close affordance (Cancel/Close) for consistency.
		if (auto hb = dynamic_cast<Gtk::HeaderBar*>(get_titlebar()))
