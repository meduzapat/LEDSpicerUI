...
protected:
	...
	/**
	 * Shows the name prompt in a retry loop, validating uniqueness within parentFsId.
	 * Loops with the previous text pre-filled when the name is taken; cancel/unchanged
	 * returns empty.
	 * @param parentFsId fsId of the parent directory the name must be unique within.
	 * @param initial    Pre-fill text (empty for new directory, current name for rename).
	 * @return Accepted unique name, or empty on cancel/unchanged.
	 */
	string promptUniqueDirName(const string& parentFsId, const string& initial) noexcept;
