...
public:
	...
	/**
	 * Creates a Link to target inside destination and decorates the BoxButton.
	 * Caller is responsible for calling setUp() first. Dedup: no-op when a
	 * Link to target already exists in destination.
	 * @param target Data to point at.
	 */
	void createLink(Storage::Data* target) noexcept;
