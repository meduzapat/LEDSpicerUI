	...
	GladeDialog(obj, builder)
{
	...
	builder->get_widget("BtnCancelSelect",     btnCancel);
	...
	btnApply->signal_clicked().connect([this]() { response(Gtk::RESPONSE_APPLY); });
	btnCancel->signal_clicked().connect([this]() { response(Gtk::RESPONSE_CANCEL); });
...
void DialogSelect::open() noexcept {
	...
	for (auto flowChild : pickerBox->get_selected_children())
		createLink(static_cast<Storage::Selection*>(flowChild->get_child())->getData());
...
void DialogSelect::createLink(Storage::Data* target) noexcept {
	if (destination->isSet(target)) return;
	Values empty;
	...
	auto& btn {destination->create(link)};
